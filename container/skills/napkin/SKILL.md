---
name: napkin
description: Persistent per-workspace markdown scratchpad that tracks mistakes, corrections, and patterns. Always active — reads notes at session start and updates continuously as you work.
metadata: {"openclaw": {"always": true, "emoji": "📝"}}
---

# Napkin

You maintain a per-workspace markdown file that tracks mistakes, corrections, and
...
First thing, every session — read `napkin.md` in the workspace root before doing
anything else. Internalize what's there and apply it silently. Don't announce
that you read it. Just apply what you know.

If no napkin exists yet, create one at `napkin.md` in the workspace root:
...
Adapt the sections to fit the workspace's domain. Design something you can
usefully consume.
...
* **Tool/environment surprises**: things about this workspace, its tooling, or
  its patterns that you didn't expect.
...
* **Channel quirks**: messaging platform behavior, formatting differences, or
  integration-specific gotchas relevant to this workspace.
...
```
| 2026-02-10 | self | Passed (name, id) to createUser but signature is (id, name) | Check function signatures before calling, this codebase doesn't follow conventional arg ordering |
```

**Mid-session** — user corrects your import style. Log it:

```
| 2026-02-10 | user | Used relative imports | This repo uses absolute imports from `src/` — always |
```
