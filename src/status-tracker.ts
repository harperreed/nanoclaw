// ABOUTME: Forward-only emoji state machine for message lifecycle signaling.
// ABOUTME: Tracks messages through RECEIVED→THINKING→WORKING→DONE/FAILED with persistent state and retry.
...
export class StatusTracker {
   ...
   * Call this after channels connect so recovery reactions can actually be sent.
   */
  ...
  private transition(
    ...
    msg.state = newState;
    // Reset trackedAt on THINKING so heartbeat timeout measures from container start
  ...
  private enqueueSend(msg: TrackedMessage, emoji: string): void {
    msg.sendChain = msg.sendChain.then(async () => {
      for (let attempt = 1; attempt <= REACTION_MAX_RETRIES; attempt++) {
        try {
          await this.deps.reactToMessage(
            msg.chatJid,
            msg.messageId,
            emoji,
            msg.fromMe,
